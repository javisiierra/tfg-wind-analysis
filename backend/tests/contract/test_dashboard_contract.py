...
def test_meteo_summary_contract(monkeypatch):
    ...
    resp = client.post("/api/v1/dashboard/meteo-summary", json={"year": 2024})
...
def test_wind_timeseries_contract(monkeypatch):
    ...
    resp = client.post("/api/v1/dashboard/wind-timeseries", json={"year": 2024})
    assert resp.status_code == 200
    body = resp.json()
    assert len(body) == 12
...
def test_wind_rose_contract(monkeypatch):
    ...
    resp = client.post("/api/v1/dashboard/wind-rose", json={"year": 2024})
    assert resp.status_code == 200
    body = resp.json()
    assert len(body) == 16
